        ...
        if method == 'initialize':
            ...
            tools = [{
                ...
                'inputSchema': {
                    'type': 'object',
                    'properties': {
                        'year': {'type': 'integer', 'description': 'Year of the data, e.g. 2020'},
                        'dataset': {'type': 'string', 'description': "Dataset endpoint, e.g. 'acs/acs5'"},
                        'get': {'type': 'array', 'items': {'type': 'string'}, 'description': 'Variables to retrieve'},
                        'for': {'type': 'string', 'description': "Geography spec, e.g. 'state:06'"}
                    },
            ...
            if missing:
                error = {'code': -32602, 'message': f"Missing parameters: {', '.join(missing)}"}
                resp = {'jsonrpc': '2.0', 'id': id_val, 'error': error}
                self._send_response(resp)
                return
            year = args['year']
