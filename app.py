...
async def jsonrpc_endpoint(request: Request, auth: bool = Depends(verify_api_key)):
    ...
    for call in requests:
        try:
            ...
            responses.append(JSONRPCResponse(id=call.get("id"), error=err).dict(exclude_none=True))
...
async def handle_rpc(req: JSONRPCRequest) -> JSONRPCResponse:
    if req.jsonrpc != "2.0":
        return JSONRPCResponse(id=req.id, error=JSONRPCError(code=-32600, message="Invalid JSON-RPC version"))
    method = req.method
    ...
    if method == "tools/call":
        ...
        if name != "census/get":
            return JSONRPCResponse(id=req.id, error=JSONRPCError(code=-32601, message=f"Unknown tool: {name}"))
        missing = [p for p in ("year", "dataset", "get", "for") if p not in args]
        if missing:
            return JSONRPCResponse(id=req.id, error=JSONRPCError(code=-32602, message=f"Missing parameters: {missing}"))
        year = args["year"]
        ...
        if not key:
            return JSONRPCResponse(id=req.id, result={"content":[{"type":"text","text":"Census API key not provided"}],"isError":True})
        url = f"https://api.census.gov/data/{year}/{dataset}"
        try:
            data = fetch_census_data(url, {"get": ",".join(get_vars), "for": geo, "key": key})
            ...
            return JSONRPCResponse(id=req.id, result={"content":[{"type":"text","text":msg}],"isError":True})
        except Exception as e:
            return JSONRPCResponse(id=req.id, result={"content":[{"type":"text","text":str(e)}],"isError":True})
    # method not found
    return JSONRPCResponse(id=req.id, error=JSONRPCError(code=-32601, message=f"Method not found: {method}"))
